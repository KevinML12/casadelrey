      ...
      <div className="flex items-center gap-6">
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={form.is_active}
            ...
            className="rounded border-outline-var" />
      ...
      <div className="flex items-start justify-between mb-8 gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-pri-con flex items-center justify-center shrink-0">
            <span className="ms text-on-pri-con" style={{ fontSize: 22 }}>campaign</span>
          </div>
          <div>
      ...
      {loading ? (
        ...
        <div className="bg-surf-low border border-outline-var rounded-2xl flex flex-col items-center py-20 gap-4 text-on-surf-var">
          <div className="w-16 h-16 rounded-[28px] bg-surf-high flex items-center justify-center">
            <span className="ms" style={{ fontSize: 32 }}>campaign</span>
          </div>
        ...
        <div className="bg-surf-low border border-outline-var rounded-2xl overflow-hidden divide-y divide-outline-var">
          {items.map(a => (
            <div key={a.ID} className="flex items-start gap-4 p-5 hover:bg-surf-high transition-colors group">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${a.is_active ? 'bg-pri-con' : 'bg-surf-high'}`}>
                <span className={`ms ${a.is_active ? 'text-on-pri-con' : 'text-on-surf-var'}`} style={{ fontSize: 18 }}>campaign</span>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="text-body-l text-on-surf font-medium">{a.title}</span>
                  <span className="text-label-s text-on-surf-var px-2 py-0.5 rounded-full bg-surf-high">
                    {targetLabel(a.role_target)}
                  </span>
                  {!a.is_active && (
                    <span className="text-label-s text-on-surf-var px-2 py-0.5 rounded-full bg-surf-high">Inactivo</span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                <IconButton onClick={() => { setEditing(a); setShowForm(true); }} title="Editar">
                  <span className="ms text-on-surf-var" style={{ fontSize: 16 }}>edit</span>
                </IconButton>
