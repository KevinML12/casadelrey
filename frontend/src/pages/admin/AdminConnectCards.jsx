      ...
      <div className="flex items-start justify-between mb-8 gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-pri-con flex items-center justify-center shrink-0">
            <span className="ms text-on-pri-con" style={{ fontSize: 22 }}>contact_page</span>
          </div>
          <div>
      ...
      {loading ? (
        ...
        <div className="bg-surf-low border border-outline-var rounded-2xl flex flex-col items-center py-20 gap-4 text-on-surf-var">
          <div className="w-16 h-16 rounded-[28px] bg-surf-high flex items-center justify-center">
            <span className="ms" style={{ fontSize: 32 }}>contact_page</span>
          </div>
        ...
        <div className="bg-surf-low border border-outline-var rounded-2xl overflow-hidden divide-y divide-outline-var">
          {cards.map(card => {
            ...
            return (
              <div key={card.ID} className="flex items-start gap-4 p-5 hover:bg-surf-high transition-colors">

                <div className="w-10 h-10 rounded-xl bg-surf-high flex items-center justify-center shrink-0 mt-0.5">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-body-s text-on-surf-var">
                    {card.phone && (
                      <span className="flex items-center gap-1.5">
                        <span className="ms" style={{ fontSize: 14 }}>phone</span>{card.phone}
                      </span>
                    )}
                    {card.email && (
                      <span className="flex items-center gap-1.5">
                        <span className="ms" style={{ fontSize: 14 }}>mail</span>{card.email}
                      </span>
                    )}
                    {card.how_found && (
                      <span className="flex items-center gap-1.5">
                        <span className="ms" style={{ fontSize: 14 }}>campaign</span>
                  ...
                  <div className="flex items-center gap-2 flex-wrap mt-3">
                    {isAdmin && (
                      <select
                        ...
                        className="text-label-m px-2.5 py-1.5 rounded-lg border border-outline-var bg-transparent text-on-surf-var focus:outline-none focus:border-pri"
                      >
                    ...
                    {nextStatus && (
                      <button
                        ...
                        className="text-label-m px-2.5 py-1.5 rounded-lg border border-outline-var text-pri hover:bg-pri-con transition-colors"
                      >
