...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/admin/cell-categories', icon: 'tag',               label: 'Categorías de células' },
    ...
    <div className="flex flex-col h-full glass-light rounded-[28px] overflow-hidden">
      ...
      <div className="px-5 py-6 border-b border-bg/10">
        <Link to="/admin" onClick={onClose} className="flex items-center gap-2.5 group">
          {/* Logo oficial (globo "Luz para las Naciones"), mismo patrón
              de chip que el Header público. */}
          ...
          <div className="min-w-0">
            ...
            <p className="text-10 font-extrabold uppercase tracking-widest text-bg/55 mt-0.5">Panel Admin</p>
          </div>
        </Link>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto">
        {NAV_GROUPS.map((group, gi) => (
          <div key={group.section || 'inicio'} className={gi > 0 ? 'mt-5' : ''}>
            {group.section && (
              <p className="px-3 mb-1.5 text-11 font-extrabold uppercase tracking-widest text-bg/35">
                {group.section}
              </p>
            )}
            <div className="space-y-0.5">
              {group.items.map(({ to, label, exact, badge }) => {
    ...
    <div className="admin-light relative flex h-screen bg-paper overflow-hidden text-bg">
      ...
      <div className="flex flex-col flex-1 min-w-0 relative z-10">
        ...
        <div className="md:hidden h-14 border-b border-bg/10 glass-light rounded-none flex items-center px-4 gap-3 shrink-0">
          ...
          <span className="text-15 text-bg font-extrabold tracking-tightish">Panel Admin</span>
        </div>
