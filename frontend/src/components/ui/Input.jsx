...
export default function Input({ label, error, helperText, size = 'md', className = '', ...props }) {
  ...
  return (
    <div className="w-full">
      ...
      <input className={`${fieldBase} ${sizeCls} ${errCls} ${className}`} {...props} />
...
export function Select({ label, error, helperText, options = [], placeholder = 'Selecciona una opción', className = '', ...props }) {
  const errCls = error ? 'border-err focus:border-err focus:ring-err/15' : '';
...
export function Textarea({ label, error, helperText, rows = 4, className = '', ...props }) {
  const errCls = error ? 'border-err focus:border-err focus:ring-err/15' : '';
