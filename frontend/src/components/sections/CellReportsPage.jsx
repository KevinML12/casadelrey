    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <div className="p-6 rounded-2xl bg-card border border-line">
          <h3 className="font-bold text-ink mb-3 flex items-center gap-2"><Upload size={18} /> Importar CSV</h3>
        ...
        <div className="p-6 rounded-2xl bg-card border border-line">
          <h3 className="font-bold text-ink mb-3 flex items-center gap-2"><Plus size={18} /> Entrada manual</h3>
          <p className="text-sm text-ink-3 mb-4">Agrega filas o usa el formulario. Mismo formato que el CSV.</p>
      ...
      <div className="mb-8 p-6 rounded-2xl bg-card border border-line">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={addRow} className="text-sm px-3 py-1.5 rounded-lg bg-bg border border-line hover:border-blue/30 text-ink-2">+ Fila</button>
                ...
                <tr key={i} className="border-b border-line last:border-0">
                  ...
                  <td className="py-2 pr-2">
                    <input value={row.cell_name} onChange={e => updateRow(i, 'cell_name', e.target.value)}
                      className="w-full max-w-[120px] px-2 py-1 rounded border border-line bg-transparent text-ink text-xs" placeholder="Célula" />
                  </td>
                  <td className="py-2 pr-2">
                  ...
                  <td className="py-2">
                    <button onClick={() => removeRow(i)} className="text-ink-3 hover:text-red-500 p-1"><Trash2 size={14} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <details className="mb-6 p-4 rounded-xl bg-bg border border-line text-sm text-ink-2">
        ...
        <p className="text-xs mt-2">Columnas: líder, célula, fecha, asistencia, visitantes, notas.</p>
      </details>
              ...
              <div className="space-y-2">
                {stats.by_cell.map((c, i) => (
                  <div key={i} className="flex justify-between items-center py-2 border-b border-line last:border-0 text-sm">
                    ...
                    <span className="text-ink-3">{c.reports} reportes · {c.attendance} asistentes · {c.visitors} visitantes</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div key={r.ID} className="bg-card border border-line rounded-xl p-5">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    {isAdmin && <span className="text-ink-3 text-xs ml-2">— {r.leader_name}</span>}
                  </div>
