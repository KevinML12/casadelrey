...
function FieldShell({ label, error, children }) {
  return (
    <label className="block">
      <span className="block text-[13px] font-semibold text-bg/60 mb-2">{label}</span>
      {children}
      {error && <span className="block text-[12.5px] font-medium text-red-600 mt-1.5">{error.message}</span>}
    </label>
  );
}
...
export default function PrayerForm({ compact = false }) {
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center text-center py-12 animate-fade-in">
        ...
        <h3 className="text-[22px] font-bold text-bg mb-2">¡Recibida con amor!</h3>
        <p className="text-[15px] text-bg/70 max-w-xs leading-relaxed mb-6">
        ...
        <motion.button
          ...
          className="inline-flex items-center gap-2 rounded-pill glass-light-nested px-6 py-3 text-[14px] font-bold text-bg focus-ring"
        >
  ...
  return (
    <div className={compact ? '' : 'max-w-2xl mx-auto'}>
      {!compact && (
        <div className="mb-8">
          <h2 className="text-[24px] font-bold text-bg tracking-tight mb-2">Petición de Oración</h2>
          <p className="text-[15px] text-bg/70">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex items-start gap-3 p-3.5 rounded-[14px] bg-bg/5 border border-bg/10">
          ...
          <p className="text-[12.5px] text-bg/60 leading-relaxed">
        ...
        <motion.button
          ...
          className="w-full inline-flex items-center justify-center gap-2.5 rounded-pill bg-bg text-white px-6 py-4 text-[15px] font-bold focus-ring disabled:opacity-60"
        >
