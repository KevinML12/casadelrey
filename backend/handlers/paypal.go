...
// CreatePayPalOrder POST /api/v1/donations/create-paypal-order
func (h *DonationHandler) CreatePayPalOrder(c echo.Context) error {
	...
	if req.Amount < 10 || req.Name == "" {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "El monto mínimo es Q10 y el nombre es obligatorio."})
	}
	...
	if err != nil {
		...
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "No se pudo conectar con PayPal."})
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "PayPal rechazó la orden."})
	}
...
// CapturePayPalOrder POST /api/v1/donations/capture-paypal-order
func (h *DonationHandler) CapturePayPalOrder(c echo.Context) error {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "No se pudo capturar el pago."})
	}

	var existing models.Donation
	if result := h.DB.Where("payment_reference = ?", req.OrderID).First(&existing); result.Error == nil {
		h.DB.Delete(&po)
		return c.JSON(http.StatusOK, map[string]interface{}{"success": true, "message": "Donación ya registrada."})
	}
